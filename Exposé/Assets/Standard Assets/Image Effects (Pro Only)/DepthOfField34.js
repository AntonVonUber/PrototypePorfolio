...
class DepthOfField34 extends PostEffectsBase {
	...
	function FocalDistance01 (worldDist : float) : float {
		return mCamera.WorldToViewportPoint((worldDist-mCamera.nearClipPlane) * mCamera.transform.forward + mCamera.transform.position).z / (mCamera.farClipPlane-mCamera.nearClipPlane);	
	}
	...
	function OnRenderImage (source : RenderTexture, destination : RenderTexture) {	
		...
		var focal01Size : float = focalSize / (mCamera.farClipPlane - mCamera.nearClipPlane);;

		if (simpleTweakMode) {		
			focalDistance01 = objectFocus ? (mCamera.WorldToViewportPoint (objectFocus.position)).z / (mCamera.farClipPlane) : FocalDistance01 (focalPoint);
			focalStartCurve = focalDistance01 * smoothness;
			focalEndCurve = focalStartCurve;
			blurForeground = blurForeground && (focalPoint > (mCamera.nearClipPlane + Mathf.Epsilon));
		} 
		else {
			if(objectFocus) {
				var vpPoint = mCamera.WorldToViewportPoint (objectFocus.position);
				vpPoint.z = (vpPoint.z) / (mCamera.farClipPlane);
